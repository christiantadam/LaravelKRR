    ...
    btn_proses.addEventListener("click", async function () {
        ...
        try {
            // CEK PENYESUAIAN (sekali saja)
            let cek = await $.ajax({
                type: "GET",
                url: "TerimaPurchasing/cekPenyesuaianTransaksi",
                data: {
                    IdType: items[0].IdType,
                    _token: csrfToken,
                },
            });
            ...
            if (cek[0].jumlah >= 1) {
                Swal.fire({
                    icon: "error",
                    text: "Tidak Bisa DiAcc karena ada Transaksi Penyesuaian",
                });
                console.warn("❌ DIBATALKAN: Ada transaksi penyesuaian");
                console.groupEnd();
                btn_proses.disabled = false;
                return;
            }
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                try {
                    ...
                    if (res && res.success) {
                        ...
                        console.log("✅ SUCCESS:", item.IdTransaksi);
                    } else {
                        throw res;
                    }
            ...
            let summaryHtml = `
                ...
                <b>Berhasil:</b> ${successLog.length}<br>
                <b>Gagal:</b> ${errorLog.length}
            `;

            if (errorLog.length > 0) {
                summaryHtml += `<hr><b>Gagal pada ID:</b><br>`;
                errorLog.forEach(e => {
