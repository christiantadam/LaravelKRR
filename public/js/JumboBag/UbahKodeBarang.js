...
btn_kodebarang.addEventListener("click", async function (event) {
    ...
    try {
        const result = await Swal.fire({
            title: "Select a Customer",
            html: '<table id="barangTable" class="display" style="width:100%"><thead><tr><th>Nama Customer</th><th>Id_Customer</th></tr></thead><tbody></tbody></table>',
            showCancelButton: true,
            preConfirm: () => {
            ...
            if (result.isConfirmed && result.value) {
                const selectedRow = result.value;
                kodeBarang.value = selectedRow.Kode_Barang.trim();
