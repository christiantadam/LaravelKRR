...
btn_redisplay.addEventListener("click", async function (event) {
    ...
    table_bawah = $("#table_bawah").DataTable({
        ...
        destroy: true,
        ajax: {
            url: "/orderB/show/getDataHasilKg",
            dataType: "json",
            type: "GET",
            data: function (d) {
                return $.extend({}, d, {
...
btnProses.addEventListener("click", function () {
    ...
    if (modeProses == 2) {
        ...
        fetchSelect(
            "/sp-orderB/SP_1273_CIR_Cek_Pegawai/" +
            dtTanggal1.value +
            "~" +
            hidIdMesin.value +
            "~" +
...
function getPegawaiFetch(s_shift, s_id_mesin, s_id_order) {
    fetchSelect(
        "/sp-orderB/SP_1273_CIR_LIST_LogMesin~6/" +
        s_id_mesin +
        "~" +
        s_id_order +
        "~" +
...
function getCounterFetch(s_id_mesin, s_shift) {
    ...
    fetchSelect("/sp-orderB/SP_1273_CIR_List_Mesin~2/" + s_id_mesin, (data) => {
        switch (s_shift) {
            case "P":
...
function getDetailMesinFetch(s_id_mesin) {
    console.log(s_id_mesin);

    fetchSelect(
        "/sp-orderB/SP_1273_CIR_List_Mesin~4/" + s_id_mesin,
        (data) => {
            ...
            fetchSelect(
                "/sp-orderB/SP_1273_CIR_Check_Order/" + txtIdOrder.value,
                (data) => {
                    if (data[0]["Ada"] > 0) {
                        showToast(
...
function getDetailLog(s_id_log) {
    fetchSelect("/sp-orderB/SP_1273_CIR_List_Log_Mesin_PerLog/" + s_id_log, (data) => {
...
function prosesIsi(x_meter) {
    ...
    fetchSelect("/sp-orderB/SP_1273_CIR_List_Order~2/" + txtIdOrder.value, (data) => {
        ...
        const isiDB = () => {
            // Proses Isi
            fetchStatement(
                "/sp-orderB/SP_1273_CIR_Insert_Log_Mesin/" +
                dtTanggal1.value +
                "~" +
                ...
                () => {
                    ...
                    // showToast("Data berhasil disimpan!", "success");
                    $.ajax({
                        url: "/orderB/show/HasilKgLogMesin",
                        type: "GET",
                        data: {
                            _token: csrfToken,
...
function prosesKoreksi() {
    fetchStatement(
        "/sp-orderB/SP_1273_CIR_Update_Log_Mesin/" +
        slcIdLog.value.trim() +
        "~" +
        dtTanggal1.value +
        "~" +
...
function prosesHapus() {
    fetchStatement(
        "/sp-orderB/SP_1273_CIR_Delete_Log_Mesin/" + slcIdLog.value.trim(),
        (data) => {
            if (data == -1) {
                fetchSelect(
                    "/sp-orderB/SP_1273_CIR_Delete_Log_Mesin~ERROR/" +
                    slcIdLog.value.trim(),
                    (data2) => {
...
PG_btnPegawai.addEventListener("click", function () {
    ...
    if (PG_dtAwal.value !== "" && PG_dtAkhir.value !== "") {
        fetchStatement(
            "/sp-orderB/SP_1273_CIR_Transfer_Pegawai/" +
            PG_dtAwal.value +
            "~" +
            PG_dtAkhir.value,
            (data) => {
