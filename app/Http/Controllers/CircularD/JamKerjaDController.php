...
class JamKerjaDController extends Controller
{
    public function index()
    {
        $access = (new HakAksesController)->HakAksesFiturMaster('Circular D');
        return view('CircularD.koreksi.JamKerja', compact('access'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            if (empty($shift)) {
                if (empty($id_mesin)) {
                    // Kode = 1
                    DB::connection('ConnCircular')->statement('EXEC SP_1273_CIR_Insert_JamKerja @Kode = ?, @Tanggal = ?, @IdTypeMesin = ?, @JamKerja = ?', [
                        '1',
                        $tanggal,
                        $idType_mesin,
                        $jam_kerja
                    ...
                    // Kode = 2
                    DB::connection('ConnCircular')->statement('EXEC SP_1273_CIR_Insert_JamKerja @Kode = ?, @Tanggal = ?, @IdTypeMesin = ?, @IdMesin = ?, @JamKerja = ?', [
                        '2',
                        $tanggal,
                        $idType_mesin,
                        $id_mesin,
                ...
                if (empty($id_mesin)) {
                    // Kode = 3
                    DB::connection('ConnCircular')->statement('EXEC SP_1273_CIR_Insert_JamKerja @Kode = ?, @Tanggal = ?, @Shift = ?, @IdTypeMesin = ?, @JamKerja = ?', [
                        '3',
                        $tanggal,
                        $shift,
                        $idType_mesin,
                    ...
                    // Kode = 4
                    DB::connection('ConnCircular')->statement('EXEC SP_1273_CIR_Insert_JamKerja @Kode = ?, @Tanggal = ?, @Shift = ?, @IdTypeMesin = ?, @IdMesin = ?, @JamKerja = ?', [
                        '4',
                        $tanggal,
                        $shift,
                        $idType_mesin,
