...
class HistorySalahDController extends Controller
{
    public function index()
    {
        $access = (new HakAksesController)->HakAksesFiturMaster('Circular D');
        return view('CircularD.koreksi.HistorySalah', compact('access'));
    }
    ...
    public function show(Request $request, $id)
    {
        if ($id == 'cekHistoryLapsalah') {
            ...
            $lstHistory = DB::connection('ConnCircular')
                ->select('EXEC SP_1273_CIR_CEKHISTORY_LAPSALAH');

            $lstHistory1 = DB::connection('ConnCircular')
                ->select('EXEC SP_1273_CIR_CEK_HISTORYSALAH_GEDUNGD @Tgl = ?', [$tanggal]);
