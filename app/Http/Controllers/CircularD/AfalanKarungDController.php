...
class AfalanKarungDController extends Controller
{
    public function index()
    {
        $access = (new HakAksesController)->HakAksesFiturMaster('Circular D');
        return view('CircularD.transaksi.AfalanKarung', compact('access'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            DB::connection('ConnCircular')->statement(
                'EXEC Sp_Update_Afalan @Id_log = ?, @Afalan_Wa = ?, @Afalan_We = ?, @WeftEnd = ?',
                [
                    $id_log,
                    $afalan_wa,
                    $afalan_we,
    ...
    public function show(Request $request, $id)
    {
        if ($id == 'getListLogMesin') {
            ...
            $results = DB::connection('ConnCircular')
                ->select(
                    'EXEC Sp_List_Log_MEsin @Kode = ?, @Shift = ?, @Tanggal = ?',
                    [7, $shift, $tanggal]
                );
