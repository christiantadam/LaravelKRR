...
class TransferHasilMeterBController extends Controller
{
    public function index()
    {
        $access = (new HakAksesController)->HakAksesFiturMaster('Circular B');
        return view('CircularB.transaksi.TransferHasilMeter', compact('access'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($flag) {
            ...
            DB::connection('ConnCircularMojosari')->statement(
                'EXEC SP_1273_CIR_Update_Kalkulasi_Meter @idlog1 = ?, @idlog2 = ?, @id_order = ?, @id_mesin = ?, @IdKonversi = ?',
                [
                    $id_log_awal,
                    $Id_Log,
                    $Id_order,
    ...
    public function show(Request $request, $id)
    {
        if ($id == 'getData') {
            $results = DB::connection('ConnCircularMojosari')
                ->select('exec SP_1273_CIR_Maint_Transfer ?', [1]);
            // dd($results);
            $response = [];
            foreach ($results as $row) {
                // Panggil SP HitungMeter (SP_1486_CIR_LIST_TRANSF_MTR_3) untuk dapatkan hasil tambahan
                $subResults = DB::connection('ConnCircularMojosari')->select(
            ...
            $results4 = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_Maint_Transfer @Kode = ?, @Id_Mesin = ?, @Id_Order = ?, @Id_Log = ?',
            ...
            $results8 = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_Maint_Transfer @Kode = ?, @TypeMesin = ?, @NamaMesin = ?, @Kode_Barang = ?',
                [8, $sType, $sNama, $skode]
            );
            // dd($results8);
            $response = [];
            ...
            $sid_order = $request->input('Id_order');
            $results = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_Maint_Transfer @Kode = ?, @id_order = ?',
                [5, $sid_order]
            );
            ...
            // dd($request->all());
            $results = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_Maint_Transfer @Kode = ?, @A_kodebarang_warp = ?, @A_kodebarang_weft = ?, @id_order = ?, @id_mesin = ?',
            ...
            $results1 = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_Maint_Transfer @Kode = ?, @Id_Mesin = ?, @Id_Order = ?, @A_kodebarang_warp = ?',
            ...
            $results2 = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_Maint_Transfer @Kode = ?, @Id_Mesin = ?, @Id_Order = ?, @A_kodebarang_warp = ?',
            ...
            // dd($sid_order, $sMesin);
            $results = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_Maint_Transfer @Kode = ?, @id_order = ?, @NamaMesin = ?',
                [10, $sid_order, $sMesin]
            );
            // dd($results);
            ...
            $results = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_Maint_Transfer @Kode = ?, @id_order = ?, @id_mesin = ?',
            ...
            $results = DB::connection('ConnCircularMojosari')->select(
                'exec SP_1273_CIR_List_Type @Kode = ?, @IdOrder = ?, @Kd_Brg = ?',
                [1, $sid_order, $sKd_Brg]
            );
