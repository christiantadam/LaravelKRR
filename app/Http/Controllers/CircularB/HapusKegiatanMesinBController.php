...
class HapusKegiatanMesinBController extends Controller
{
    public function index()
    {
        $access = (new HakAksesController)->HakAksesFiturMaster('Circular B');
        return view('CircularB.koreksi.HapusKegiatanMesin', compact('access'));
    }
    ...
    public function show(Request $request, $id)
    {
        if ($id == 'getData') {
            ...
            $resultsIdMax = DB::connection('ConnCircularMojosari')->select($sql, ['6', $tanggal, $mesin]);
            // dd($resultsIdMax);
            $idMax = 0;
            ...
            // dd($results);
            if ($kode == '8') {
                $results = DB::connection('ConnCircularMojosari')->select('EXEC SP_1273_CIR_ERROR_CIR @Kode = ?, @Tanggal = ?, @Mesin = ?, @IdMax = ?', [
                    $kode,
                    $tanggal,
                    $mesin,
                    $idMax
                ...
                $results = DB::connection('ConnCircularMojosari')->select('EXEC SP_1273_CIR_ERROR_CIR @Kode = ?, @Tanggal = ?, @Mesin = ?', [
                    $kode,
                    $tanggal,
                    $mesin
                ]);
