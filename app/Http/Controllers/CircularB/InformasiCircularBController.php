...
class InformasiCircularBController extends Controller
{
    public function index($form_name)
    {
        ...
        switch ($form_name) {
            case 'formCounterMesin':
                ...
                break;
            default:
                return view('CircularB.informasi.' . $form_name, compact('access'));
        }

        return view('CircularB.informasi.' . $form_name, $form_data, compact('access'));
    }
    ...
    public function getLaporanHistory(Request $request)
    {
        ...
        $query = DB::connection('ConnCircularMojosari')->table(function ($query) use ($tanggal) {
