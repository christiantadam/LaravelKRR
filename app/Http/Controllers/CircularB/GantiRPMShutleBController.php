...
class GantiRPMShutleBController extends Controller
{
    public function index()
    {
        $access = (new HakAksesController)->HakAksesFiturMaster('Circular B');
        return view('CircularB.koreksi.GantiRPMShutle', compact('access'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (empty($shift)) {
                DB::connection('ConnCircularMojosari')->statement('exec SP_1273_CIR_ERROR_CIR @Kode = ?, @Tanggal = ?, @IdMesin = ?, @RPM = ?', [$kode1, $tanggal, $dataList[0]['Id_mesin'], $ganti_rpm]);
            } else {
                DB::connection('ConnCircularMojosari')->statement('exec SP_1273_CIR_ERROR_CIR @Kode = ?, @Tanggal = ?, @IdMesin = ?, @RPM = ?, @Shift = ?', [$kode1, $tanggal, $dataList[0]['Id_mesin'], $ganti_rpm, $shift]);
            }
